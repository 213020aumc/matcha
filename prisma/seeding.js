...
// ENUMS
const DIET_OPTIONS = ["OMNIVORE", "VEGETARIAN", "VEGAN", "KOSHER", "HALAL", "GLUTEN_FREE"];
...
const RACE_OPTIONS = ["Caucasian", "Asian", "African American", "Hispanic", "Mixed", "Other"];
...
async function main() {
  ...
  // 1. PLANS
  const plans = [
    { code: "BASIC", name: "Basic Access", price: 0.0, billingCycle: "MONTHLY" },
    { code: "PREMIUM", name: "Premium Family", price: 29.99, billingCycle: "MONTHLY" },
  ];
  ...
  await prisma.user.upsert({
    ...
    create: {
      ...
      // FIX: Changed from [] to null (or specific value like "SPERM")
      interestedIn: null,            
      pairingTypes: ["DONOR_BANK"],  // Keep as Array (Schema allows it)
    },
  });
  console.log("👉 Created Onboarded User: onboarded@helix.com");

  // --- STATE C: Partially Completed Profile ---
  await prisma.user.upsert({
  ...
  for (let i = 0; i < NUM_USERS; i++) {
    ...
    await prisma.user.create({
      data: {
        ...
        role: "DONOR",
        // FIX: Add default serviceType for random users too
        serviceType: "DONOR_SERVICES", 
        ...
        health: {
          create: {
            ...
            pregnancyHistory: sex === "female" ? faker.datatype.boolean() : null,
          },
        },
        genetic: {
          create: {
        ...
        compensation: {
          create: {
            ...
            askingPrice: parseFloat(faker.commerce.price({ min: 1000, max: 5000 })),
            minAcceptedPrice: parseFloat(faker.commerce.price({ min: 800, max: 1000 })),
            buyNowPrice: parseFloat(faker.commerce.price({ min: 6000, max: 10000 })),
          },
        },
        legal: {
          create: {
  ...
  const permissions = [
    ...
    { slug: "profiles.view_pending", description: "View profiles waiting for review" },
    { slug: "profiles.approve", description: "Approve or Reject profiles" },
    { slug: "profiles.view_sensitive", description: "View private health/genetic data" },
  ...
  const modPermissions = allPermissions.filter((p) =>
    p.slug.startsWith("profiles.") || p.slug === "users.view" || p.slug === "dashboard.view"
  );
  await prisma.role.upsert({
